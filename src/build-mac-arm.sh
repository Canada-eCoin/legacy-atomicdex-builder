#!/bin/bash
# ============================================================================
# build-mac-arm.sh — Native arm64 macOS build for legacy atomicdex
# ============================================================================
...
#
# Usage:
#   ./build-mac-arm.sh                  # full native arm64 build (KDF + desktop)
#   ./build-mac-arm.sh --kdf-only       # native arm64 KDF only
#   ./build-mac-arm.sh --desktop-only   # native arm64 desktop only (needs output/mac-arm/kdf)
#   ./build-mac-arm.sh --yes            # skip prompts
#   ./build-mac-arm.sh --install-deps   # install missing brew deps, then exit
...
OUTPUT_DIR="${OUTPUT_DIR:-${PROJECT_DIR}/output}/mac-arm"
LOG_DIR="${LOG_DIR:-${PROJECT_DIR}/logs}/mac-arm"
BUILD_ROOT="${BUILD_DIR:-${PROJECT_DIR}/.build}/mac-arm"
...
host_setup() {
    case "$(uname -m)" in
        arm64)
            ...
            ;;
        x86_64)
            die "build-mac-arm.sh currently requires an Apple Silicon host; use build-mac-intel.sh on Intel Macs"
            ;;
        *)
